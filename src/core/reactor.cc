...
void reactor::task_queue::do_dump() const {
    ...
    if (!seastar_logger.is_enabled(log_level::warn) || rate_limit.rate_limited()) {
        // Rate limit was hit or `warning` level is not enabled, we skip dumping the queue.
        return;
    }
    std::string dropped_messages_msg;
    if (rate_limit.has_dropped_messages()) {
        dropped_messages_msg = fmt::format(" (rate limiting dropped {} similar messages) ", rate_limit.get_and_reset_dropped_messages());
    }

    seastar_logger.log(log_level::warn, "Too long queue accumulated for {} ({} tasks){}", _name, _q.size(), dropped_messages_msg);
